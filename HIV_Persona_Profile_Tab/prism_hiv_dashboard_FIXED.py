...
# ─── Systematic missing-value recode ─────────────────────────────────
# In this study's raw .sav, non-response codes (98, 99, 998, 999) appear
# in numeric Q* variables alongside valid responses. These must be recoded
# to system-missing before any aggregation, or weighted means will be
# pulled out of their valid response ranges.
#
# Universal rule: for every Q-prefix variable, values in {98, 99, 998, 999}
# are recoded to NaN. This is safe because:
#   - Likert (1-7) and binary (0/1) response variables never legitimately
#     take values >= 98
#   - Categorical demographic variables in this codebook use small integer
#     codes (1-20 range), never 98/99 except as "Prefer not to say"
#   - Study version / segment ID variables that legitimately exceed these
#     values (e.g. QHIV_Version with max=272) do not contain literal 98/99
#     as data values either
MISSING_CODES = {98, 99, 998, 999}
_recode_log = {}
for v in df.columns:
    if v.startswith('Q'):
        mask = df[v].isin(MISSING_CODES)
        n = int(mask.sum())
        if n > 0:
            df.loc[mask, v] = float('nan')
            _recode_log[v] = n
if _recode_log:
    print(f"  recoded missing codes (98/99/998/999) to NaN in {len(_recode_log)} variables:")
    for v, n in sorted(_recode_log.items(), key=lambda x: -x[1])[:15]:
        print(f"    {v:18} {n} cases")
    if len(_recode_log) > 15:
        print(f"    ... and {len(_recode_log) - 15} more")
...
def item_row(code, stem, var, binary):
    ...
    # Per-segment means for all 16 segments
    by_segment = {}
    for sid in range(1, 17):
        sub = df[df['SEG'] == sid]
        by_segment[str(sid)] = wmean(sub[var], sub['WT']) if len(sub) > 0 else None
    return {
        'code': code, 'stem': stem,
...
for code in TRUST_LBL:
    ...
    for sid in range(1, 17):
        sub = df[df['SEG'] == sid]
        by_segment[str(sid)] = wmean(sub[code], sub['WT']) if len(sub) > 0 else None
    trust.append({
